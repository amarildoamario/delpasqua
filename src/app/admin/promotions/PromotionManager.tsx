      ...
      <div className="rounded-3xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-3">
          <div className="flex flex-col">
            <label className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Codice</label>
            <input
              value={code}
          ...
          <div className="flex flex-col">
            <label className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Tipo</label>
            <select
              value={type}
              onChange={(e) => {
          ...
          <div className="flex flex-col">
            <label className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Descrizione (opz.)</label>
            <input
              value={desc}
          ...
          {type === "percent" ? (
            <div className="flex flex-col">
              <label className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Percentuale</label>
              <input
                type="number"
                value={percent}
                min={1}
          ...
          {type === "fixed" ? (
            <div className="flex flex-col">
              <label className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Sconto fisso (cents)</label>
              <input
                type="number"
                value={amountCents}
                min={0}
                ...
                className="rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm text-neutral-900 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-100"
              />
              <div className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{euro(amountCents)}</div>
            </div>
          ) : null}

          <div className="flex flex-col">
            <label className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Min ordine (cents)</label>
            <input
              type="number"
              value={minOrderCents}
              min={0}
          ...
          <div className="flex flex-col">
            <label className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Limite utilizzi</label>
            <input
              type="number"
              value={usageLimit}
              min={0}
          ...
          <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
            {rows.map((p) => (
              <tr key={p.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-950/40">
                <td className="px-4 py-3 font-extrabold text-neutral-900 dark:text-neutral-100">{p.code}</td>
                <td className="px-4 py-3 text-neutral-700 dark:text-neutral-200">{p.type}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="inline-flex gap-2">
                    <button
                      ...
                      className="rounded-xl border border-neutral-200 bg-white px-3 py-2 text-xs font-semibold text-neutral-900 hover:bg-neutral-50 disabled:opacity-50 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100 dark:hover:bg-neutral-800"
                    >
                    ...
                    <button
                      disabled={saving}
                      onClick={() => remove(p.id)}
                      className="rounded-xl border border-red-200 bg-white px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50 dark:border-red-900/40 dark:bg-neutral-900 dark:text-red-300 dark:hover:bg-red-950/30"
                    >
                      Elimina
                    </button>
                  </div>
            ...
            {rows.length === 0 ? (
              <tr>
                <td className="px-4 py-6 text-neutral-600 dark:text-neutral-300" colSpan={7}>
