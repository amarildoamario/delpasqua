    ...
    <div className="overflow-hidden">
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
            ...
            return (
              <tr key={p.id} className="text-neutral-800 dark:text-neutral-200">
                <td className="px-6 py-4">
                  ...
                  {p.description ? (
                    <div className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{p.description}</div>
                  ) : null}
                </td>
                ...
                <td className="px-6 py-4">
                  {p.freeShipping ? (
                    <span className="inline-flex items-center rounded-full border border-neutral-200 bg-white px-2 py-1 text-xs font-bold text-neutral-900 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-100">
                      Gratis
                    </span>
                  ) : (
                    <span className="text-neutral-500 dark:text-neutral-400">Normale</span>
                  )}
                </td>

                <td className="px-6 py-4 text-neutral-600 dark:text-neutral-300">{fmt(p.createdAt)}</td>

                <td className="px-6 py-4">
                  <div className="text-neutral-600 dark:text-neutral-300">{fmt(p.endsAt)}</div>
                  {isExpiring && p.startsAt ? (
                    <div className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">dal {fmt(p.startsAt)}</div>
                  ) : null}
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <button
                    ...
                    className="rounded-2xl border border-neutral-200 bg-white px-4 py-2 text-sm font-extrabold text-neutral-900 shadow-sm hover:bg-neutral-50 disabled:opacity-60 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100 dark:hover:bg-neutral-800"
                  >
          ...
          {!sorted.length && (
            <tr>
              <td colSpan={7} className="px-6 py-10 text-sm text-neutral-500 dark:text-neutral-400">
