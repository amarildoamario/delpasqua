...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-[11px] font-extrabold uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
      {children}
    </div>
  );
}
  ...
  return (
    <input
      ...
      className={[
        ...
        "focus:ring-2 focus:ring-neutral-900/10 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-100 dark:focus:ring-white/10",
        className,
      ].join(" ")}
    />
  );
...
function CheckRow({
  ...
  return (
    <label className="flex items-center justify-between gap-3 rounded-2xl border border-neutral-200 bg-white px-3 py-2 dark:border-neutral-800 dark:bg-neutral-950">
...
export default function SalesTable({ rows }: { rows: Row[] }) {
  ...
  if (!rows.length) {
    return (
      <div className="rounded-2xl border border-neutral-200 bg-white p-6 text-sm text-neutral-600 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-300">
    ...
    <div className="space-y-4">
      {rows.map((r) => {
        ...
        return (
          <div
            key={r.productKey}
            className="rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900"
          >
            {/* Header */}
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="truncate text-base font-extrabold text-neutral-900 dark:text-neutral-100">{r.title}</h3>
                  {s.badge ? (
                    <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-extrabold text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
                      {BADGES.find((b) => b.value === s.badge)?.label ?? s.badge}
                    </span>
                  ) : null}
                  {s.isBestSeller ? (
                    <span className="rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-extrabold text-sky-700 dark:bg-sky-950/40 dark:text-sky-200">
                      Best seller
                    </span>
                  ) : null}
                  {s.showInHome ? (
                    <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-extrabold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-200">
                      In home
                    </span>
                  ) : null}
                </div>

                <div className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">/shop/{r.slug}</div>

                <div className="mt-3 flex flex-wrap items-end gap-4">
                  <div>
                    <div className="text-xs text-neutral-500 dark:text-neutral-400">Base</div>
                  ...
                  <div>
                    <div className="text-xs text-neutral-500 dark:text-neutral-400">Promo</div>
                    <div className="text-sm font-extrabold text-neutral-900 dark:text-neutral-100">
              ...
              <div className="flex items-center gap-2">
                {isSaved ? (
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-extrabold text-emerald-800 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-200">
                    Salvato ✅
                  </div>
                ) : null}
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="rounded-2xl bg-neutral-900 px-4 py-2 text-sm font-extrabold text-white shadow-sm hover:opacity-90 disabled:opacity-60 dark:bg-white dark:text-neutral-900"
                >
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {/* Badge + Promo label */}
              <div className="rounded-3xl border border-neutral-200 bg-neutral-50 p-3 dark:border-neutral-800 dark:bg-neutral-950/40">
              ...
              <div className="rounded-3xl border border-neutral-200 bg-neutral-50 p-3 dark:border-neutral-800 dark:bg-neutral-950/40">
                ...
                <div className="mt-3 grid gap-3">
                  ...
                  <div className="rounded-2xl border border-neutral-200 bg-white px-3 py-2 text-xs font-semibold text-neutral-700 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-200">
