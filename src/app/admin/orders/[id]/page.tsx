...
import ShipToggleButton from "@/app/admin/orders/ship/ShipToggleButton";
import OrderStatusActions from "@/app/admin/orders/[id]/OrderStatusActions";
...
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  subtitle={
    <div className="space-y-1">
      <div className="text-xs text-neutral-500 dark:text-neutral-400 print-hidden">
...
/>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Cliente</div>
          <div className="mt-2 text-sm text-neutral-900 dark:text-neutral-100">
            <div className="font-semibold">{order.fullName}</div>
            <div className="text-neutral-600 dark:text-neutral-300">{order.email}</div>

            <div className="mt-3 text-neutral-700 dark:text-neutral-200">
        ...
        {/* Totali */}
        <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Totali</div>
          <div className="mt-3 space-y-1 text-sm text-neutral-800 dark:text-neutral-200">
        ...
        {/* Spedizione */}
        <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Spedizione</div>
          ...
          {order.status !== "PREPARING" ? (
            <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-200">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-2">
          <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Azioni ordine</div>
          <div className="mt-3">
            <OrderStatusActions
              orderId={order.id}
              status={order.status}
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Audit & Timeline</div>
            ...
            {order.items.map((it: Item) => (
              <tr key={it.id}>
                <td className="px-4 py-3">
                  <div className="font-semibold text-neutral-900 dark:text-neutral-100">{it.title}</div>
                  <div className="text-xs text-neutral-500 dark:text-neutral-400">
                    productId: {it.productId} • variantId: {it.variantId}
                  </div>
                </td>
                <td className="px-4 py-3 text-neutral-700 dark:text-neutral-200">{it.variantLabel}</td>
                <td className="px-4 py-3 text-right text-neutral-700 dark:text-neutral-200">{euro(it.unitPriceCents)}</td>
                <td className="px-4 py-3 text-right text-neutral-700 dark:text-neutral-200">{it.qty}</td>
...
<div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
  <div className="flex items-center justify-between gap-3">
    <div>
      <div className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">Storico azioni</div>
    ...
    {order.events.length ? (
      order.events.map((ev) => {
        ...
        return (
          <div
            key={ev.id}
            className="rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm dark:border-neutral-800 dark:bg-neutral-950"
          >
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="flex items-center gap-2">
                <span className="inline-flex rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-semibold text-neutral-700 dark:bg-neutral-800 dark:text-neutral-200">
                  {ev.type}
                </span>
                {transition ? (
                  <span className="text-xs font-semibold text-neutral-600 dark:text-neutral-300">{transition}</span>
                ) : null}
              </div>
            ...
            {ev.message ? (
              ...
              <div className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">—</div>
            )}
            ...
            {ev.metaJson ? (
              <details className="mt-2">
                ...
                <pre className="mt-2 overflow-x-auto rounded-xl bg-neutral-50 p-3 text-xs text-neutral-800 dark:bg-neutral-900 dark:text-neutral-200">
      ...
      <div className="text-sm text-neutral-500 dark:text-neutral-400">Nessun evento registrato.</div>
    )}
  </div>
</div>
...
function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={strong ? "font-bold text-neutral-900 dark:text-neutral-100" : "text-neutral-900 dark:text-neutral-100"}>
        {value}
      </span>
    </div>
  );
