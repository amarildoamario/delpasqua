...
export default async function AdminOrdersPage({
  ...
  const jsonHref = `/api/admin/invoices.json?mode=range&start=${startISO}&end=${endISO}${
    ...
    <div className="space-y-4">
        ...
        actions={
          <>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-xl bg-neutral-900 px-3 py-2 text-sm font-semibold text-white hover:opacity-90 dark:bg-white dark:text-neutral-900"
            >
              Scarica fatture (.JSON)
            </a>
      ...
      <form className="flex flex-wrap items-end gap-2" action="/admin/orders" method="get">
        ...
        <input
          ...
          className="w-64 rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm text-neutral-900 outline-none focus:ring-2 focus:ring-neutral-900/20 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100 dark:focus:ring-white/20"
        />

        <select
          name="status"
          defaultValue={statusParam ?? ""}
          className="rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm text-neutral-900 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
        >
        ...
        <button className="rounded-xl bg-neutral-900 px-3 py-2 text-sm font-semibold text-white hover:opacity-90 dark:bg-white dark:text-neutral-900">
          Filtra
        </button>
      </form>

      <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-neutral-200/60 dark:bg-neutral-900 dark:ring-neutral-800/60">
        <table className="w-full table-fixed text-left text-sm">
          ...
          <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
              ...
              return (
                <tr
                  ...
                  className={[
                    ...
                    o.status === "PREPARING" ? "bg-sky-100 dark:bg-sky-950/45" : "",
                    o.status === "SHIPPED" ? "bg-emerald-100 dark:bg-emerald-950/45" : "",
                  ].join(" ")}
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    {exp ? (
                      <div className="mt-1 truncate text-xs font-semibold text-red-700 dark:text-red-300">
                  ...
                  <td className="px-4 py-3 text-neutral-700 dark:text-neutral-200">
                    ...
                    <div className="truncate text-xs text-neutral-500 dark:text-neutral-400">{o.email || "—"}</div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={[
                        ...
                        o.status === "PREPARING"
                          ? "bg-sky-200 text-sky-900 dark:bg-sky-950/60 dark:text-sky-100"
                          : o.status === "SHIPPED"
                          ? "bg-emerald-200 text-emerald-900 dark:bg-emerald-950/60 dark:text-emerald-100"
                  ...
                  <td className="px-4 py-3">
                    {o.isFlagged ? (
                      <span className="inline-flex rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold text-red-700 dark:bg-red-950/40 dark:text-red-300">
                        FLAG {o.riskScore}
                      </span>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    {o.shippedAt ? (
                      <span className="inline-flex rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700 dark:bg-sky-950/40 dark:text-sky-300">
                        Sì
                      </span>
                    ) : (
                      <span className="inline-flex rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-semibold text-neutral-700 dark:bg-neutral-800 dark:text-neutral-200">
            ...
            {orders.length === 0 ? (
              <tr>
                <td className="px-4 py-6 text-neutral-600 dark:text-neutral-300" colSpan={9}>
