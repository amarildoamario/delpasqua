...
function Section({
  ...
  return (
    <div className="rounded-3xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
      <div className="mb-4">
        <div className="text-base font-bold text-neutral-900 dark:text-neutral-100">{title}</div>
...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-800 dark:bg-neutral-950/40">
      <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">{title}</div>
      <div className="mt-1 text-2xl font-extrabold tracking-tight text-neutral-900 dark:text-neutral-100">
        {value}
      </div>
...
const [totalOrders, paidOrders, pendingOrders, canceledOrders] = await Promise.all([
  ...
  prisma.order.count({ where: { createdAt: { gte: start, lt: endExclusive }, status: "PENDING" } }),
  prisma.order.count({ where: { createdAt: { gte: start, lt: endExclusive }, status: "CANCELED" } }),
]);
...
>`
  ...
  FROM "Order"
  WHERE createdAt >= ${start.toISOString()} AND createdAt < ${endExclusive.toISOString()}
  GROUP BY day
  ORDER BY day ASC
`) ?? [];
  ...
  actions={
    <Link
      ...
      className="rounded-xl border border-neutral-200 bg-white px-4 py-2 text-sm font-semibold text-neutral-900 hover:bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100 dark:hover:bg-neutral-800"
    >
      Export CSV
    </Link>
  }
            ...
            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
              {topProducts.map((p) => (
                <tr key={p.rowKey}>
                  ...
                  <td className="py-2 pr-4 font-semibold text-neutral-900 dark:text-neutral-100">{p.title}</td>
                  <td className="py-2 pr-4 text-neutral-600 dark:text-neutral-300">{p.variantLabel}</td>
                  <td className="py-2 pr-4 text-neutral-700 dark:text-neutral-200">{Number(p.qty)}</td>
                  <td className="py-2 pr-4 text-right font-extrabold text-neutral-900 dark:text-neutral-100">
              ...
              {topProducts.length === 0 ? (
                <tr>
                  <td className="py-6 text-neutral-600 dark:text-neutral-300" colSpan={4}>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.day}>
                    <td className="py-2 pr-4 font-semibold text-neutral-900 dark:text-neutral-100">{r.day}</td>
                    <td className="py-2 pr-4 text-neutral-700 dark:text-neutral-200">{r.orders}</td>
                    <td className="py-2 pr-4 text-neutral-700 dark:text-neutral-200">{r.paidOrders}</td>
                    <td className="py-2 pr-4 font-extrabold text-neutral-900 dark:text-neutral-100">
                      {euro(paidGross)}
                    </td>
                    <td className="py-2 pr-4">
                      <div className="h-2 w-[240px] rounded-full bg-neutral-100 dark:bg-neutral-800">
                        <div className="h-2 rounded-full bg-neutral-900 dark:bg-white" style={{ width: `${w}px` }} />
                      </div>
                    </td>
                  </tr>
                );
              ...
              {rows.length === 0 ? (
                <tr>
                  <td className="py-6 text-neutral-600 dark:text-neutral-300" colSpan={5}>
