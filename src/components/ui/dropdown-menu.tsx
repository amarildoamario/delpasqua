      ...
      <Menu.Positioner sideOffset={sideOffset} align={align}>
        <Menu.Popup
          ...
          className={({ transitionStatus }) =>
            cn(
              "z-[80] min-w-32 overflow-hidden rounded-2xl border border-stone-200/80 bg-white/95 p-1.5 text-stone-900 shadow-2xl backdrop-blur-xl outline-none",
  ...
  ({ className, ...props }, ref) => (
    <Menu.Item
      ...
      className={({ highlighted, disabled }) =>
        cn(
          "flex cursor-default items-center gap-2 rounded-xl px-3 py-2.5 text-xs font-medium uppercase tracking-[0.14em] outline-none transition-colors",
  ...
  ({ className, ...props }, ref) => (
    <Menu.LinkItem
      ...
      className={({ highlighted }) =>
        cn(
          "flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs font-medium uppercase tracking-[0.14em] outline-none transition-colors",
          highlighted ? "bg-stone-100 text-stone-950" : "text-stone-600",
          className
        )
      }
